...
class Base:
    ...
    def load_from_file(cls):
        """ class method that returns a list of instances from JSON file """

        with open("{}.json".format(cls.__name__)) as new_file:
            if not new_file:
                return []

            json_list = Base.from_json_string(new_file.read())
            instances_list = []
